class FsstWasm {
    constructor() {
        this.module = null;
        this.createEncoderFn = null;
        this.freeEncoderFn = null;
        this.exportEncoderFn = null;
        this.importDecoderFn = null;
    ...
    createEncoder(samples, zeroTerminated = false) {
        ...
        // Create the encoder
        const encoderPtr = this.createEncoderFn(stringsPtr, lengthsPtr, samples.length, zeroTerminated ? 1 : 0);
        ...
        return {
            compress(data) {
                ...
                // Compress the data
                const outputLength = compressFn(encoderPtr, inputPtr, data.length, outputPtr, outputMaxLength);
    ...
    createDecoder(encoderData) {
        ...
        return {
            decompress(data, maxOutputSize = 0) {
                ...
                module.writeArrayToMemory(data, inputPtr);
                // Decompress
                const outputLength = decompressFn(decoderPtr, inputPtr, data.length, outputPtr, maxOutputSize);
                if (outputLength === 0) {
                    module._free(inputPtr);
                    module._free(outputPtr);
                    throw new Error("Decompression failed: buffer too small or invalid data");
                }
                // Copy result
                const result = new Uint8Array(outputLength);
                result.set(module.HEAPU8.subarray(outputPtr, outputPtr + outputLength));
